"""Script CPI-007: Idioma Ingles - FVCL Format"""
...
def run_test():
    print("=" * 60)
    print("CPI-007: Idioma Ingles (ENG)")
    print("=" * 60)
    now = datetime.now()
    ...
    # Paso 2: Search Installments con language=ENG
    ...
    r2 = requests.post(CONFIG["installments_url"], headers={"Accept": "application/json", "Content-Type": "application/json",
                       ...
                       json={"bin": "545545", "merchantCode": CONFIG["merchant_code"], "language": "ENG"}, timeout=30)
...
if __name__ == "__main__":
    ...
    with open(os.path.join(os.path.dirname(__file__), "test_report.txt"), "w", encoding="utf-8") as f:
        f.write(f"CPI-007: Idioma Ingles\n")
        f.write(f"Estado: PASO\n" if passed else "Estado: FALLO\n")
