...
Descripción:
    ...
    2. Búsqueda de cuotas disponibles para un BIN específico
...
def generate_transaction_id():
    """
    Genera un transactionId único con formato: FVCL + YYYYMMDDHHMMSS
    Ejemplo: FVCL20251029075105
    """
    now = datetime.now()
    return f"FVCL{now.strftime('%Y%m%d%H%M%S')}"
...
def run_test():
    ...
    print(f"Ambiente: QA")
    print(f"Formato TransactionId: FVCL + YYYYMMDDHHMMSS")
    print("=" * 60)
    ...
    # ⏱️ CRÍTICO: Delay de 2 segundos para SANDBOX
    # Este delay es necesario para evitar Error TN "El token no debe ser nulo o vacío"
    # SANDBOX requiere tiempo para procesar el token antes de usarlo
    print("\n⏱️  Esperando 2 segundos antes de buscar cuotas (requerido por SANDBOX)...")
    time.sleep(2)
